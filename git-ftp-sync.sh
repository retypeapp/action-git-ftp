...
missing_required_arg=false
if [ -z "${INPUT_FTP_HOST}" ]; then
  echo "Required parameter 'ftp-host' not provided."
  missing_required_arg=true
fi
if [ -z "${INPUT_FTP_PASS}" ]; then
  echo "Required parameter 'ftp-pass' not provided."
  missing_required_arg=true
fi
if [ -z "${INPUT_FTP_ROOT}" ]; then
  echo "Required parameter 'ftp-root' not provided."
  missing_required_arg=true
fi
if [ -z "${INPUT_FTP_USER}" ]; then
  echo "Required parameter 'ftp-user' not provided."
  missing_required_arg=true
fi

if ${missing_required_arg}; then
 fail "One or more required parameters are missing."
fi

if ! which git-ftp > /dev/null 2>&1; then
  fail "The git-ftp tool is not available. It is supposed to be natively available to GitHub Actions virtual environments."
fi
...
if [ "${#result}" -eq 0 ]; then
  # different to the github-pages action, we'll carry on with the process as to allow
  # git-ftp itself to determine whether changes are present.
  # This can happen if both github-pages and git-ftp actions are used in sequence.
  echo "no changes."
elif [ ! -z "${config_id}" -a \
  ...
  if [ -z "$(git status --porcelain . 2>&1)" ]; then
    echo -n "config.js ID change only, reverting"
    git checkout -- "${configjs_path}"
    echo ", done."
  else
...
if [ "$(pwd)" != "${orgwd}" ]; then
 echo "Switching back to repository root."
 cd "${orgwd}"
fi
...
echo "::group::git ftp push ..."
echo "${result}"
echo "::endgroup::"
if [ ${retstat} -eq 0 ]; then
  echo "Successfully synchronized built documentation with FTP host (differential push)."
else
  # Check if git-ftp failed just because the remote directory is not properly initialized
  if echo "${result}" | egrep -q "fatal: Could not get last commit. (|Network down\? Wrong URL\? )Use 'git ftp init' for the initial push."; then
    echo "The remote directory appears not to be initialized. Trying to initialize it..."
    cmdln[2]="init"
    ...
    if [ ${retstat} -eq 0 ]; then
      echo "::group::git ftp init ..."
      echo "${result}"
      echo "::endgroup::"
      echo "Successfully synchronized built documentation with FTP host through initialization (full push)."
    else
      fail_cmd false "Unable to synchronize-init built documentation to FTP host" "${cmdln[*]}" "${result}"
    fi
  else
    fail_cmd false "Unable to synchronize-push built documentation to FTP host" "${cmdln[*]}" "${result}"
  fi
fi